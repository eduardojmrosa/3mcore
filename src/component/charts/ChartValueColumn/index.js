...
function ChartValueColumn() {
    ...
    useEffect(() => {
        // Aqui você pode atualizar qualquer estado ou realizar outras ações necessárias
        // com base nos cards filtrados
    }, [filteredCards]);
    ...
    const data = [
        ...
        ...listaColumns.map((column) => {
            const columnCards = filteredCards.filter((card) => card.id_column === column.id);
            const columnValue = columnCards.reduce((total, card) => total + card.valor, 0);
